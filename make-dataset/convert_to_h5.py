...
def to_np_array(ak_array, maxN=100, pad=0, dtype=np.float16):
...
def store_objects(arrays, nentries, nobj=10, obj='jet'):
    ...
    l1Obj_cart = np.zeros((nentries, maxobj, 3), dtype=np.float16)
...
def convert_to_h5(input_file, output_file, tree_name, uGT_tree_name):
    ...
    l1sum_cart = np.zeros((nentries, 3), dtype=np.float16)
